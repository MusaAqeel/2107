    ...
    with patch("requests.get") as mock_get, patch("requests.post") as mock_post:
        def side_effect_get(*args, **kwargs):
            ...
            if "search" in url:
                ...
                return Mock(status_code=200, json=lambda: {"id": "test_user_id"})
...
def mock_gpt_response():
    with patch("openai.OpenAI") as mock:
        recommendations_obj = RecommendationsResponse.model_validate({
            "recommendations": [
        ...
        mock_instance.beta.chat.completions.parse.return_value.choices = [
            Mock(message=Mock(parsed=recommendations_obj))
        ]
        yield mock
...
def test_gpt_error_handling():
    with patch("main.get_gpt_recommendations", side_effect=Exception("Invalid GPT response")):
        response = client.post("/api/generate",
            json={"prompt": "test", "auth_token": "test_token", "playlist_length": 5})
...
def test_spotify_integration(mock_spotify):
    ...
    data = response.json()
    assert "track_ids" in data
    assert isinstance(data["track_ids"], list)
